...
import { belongsToAcademy, getStoredActiveAcademyId } from "./academyTenancy.js";
 ...
 * @property {string} forgePersonId
 * @property {string} forgeAcademyStudentId
...
export function mapStudent(id, data) {
  ...
  return {
    ...
    rmsPersonId: data.rmsPersonId ?? "",
    forgePersonId: data.forgePersonId ?? data.personId ?? "",
    forgeAcademyStudentId: data.forgeAcademyStudentId ?? "",
    forgeDepartmentId: data.forgeDepartmentId ?? "",
...
/** @param {string} [academyId] @returns {Promise<StudentRecord[]>} */
export async function listStudents(academyId = getStoredActiveAcademyId()) {
  const snap = await getDocs(query(studentsRef));
  const students = snap.docs
    .map((item) => mapStudent(item.id, item.data()))
    .filter(Boolean)
    .filter((student) => belongsToAcademy(student, academyId));
...
/**
 * Resolve permanent forgePersonId via Integration Hub before local create.
 * @param {ReturnType<typeof sanitizeStudentPayload>} payload
 */
async function resolveForgePersonIdForStudent(payload) {
  const callable = httpsCallable(functions, "resolveForgePersonIdentityCallable");
  const result = await callable({
    femaSid: payload.femaSid,
    firstName: payload.firstName,
    lastName: payload.lastName,
    email: payload.email,
    dateOfBirth: payload.dateOfBirth,
    rmsPersonId: payload.rmsPersonId || undefined,
    sourceSystem: "academy",
  });
  const data = result.data || {};
  if (data.hubConfigured === false) {
    return { forgePersonId: null, hubConfigured: false };
  }
  if (data.review) {
    throw new IdentityReviewRequiredError(data.review);
  }
  const forgePersonId = String(data.forgePersonId || data.personId || data.identity?.forgePersonId || "").trim();
  if (!forgePersonId) {
    throw new Error("Hub did not return a forgePersonId.");
  }
  return { forgePersonId, hubConfigured: true, created: Boolean(data.created) };
}
 ...
 */
export async function createStudent(input, createdByUid, academyId = getStoredActiveAcademyId()) {
  ...
  let forgePersonId = "";
  try {
    const resolved = await resolveForgePersonIdForStudent(payload);
    forgePersonId = resolved.forgePersonId || "";
  } catch (err) {
    if (err instanceof IdentityReviewRequiredError) throw err;
    // Hub misconfiguration / network: still block inventing a local personId.
    // Local duplicate checks already ran; allow create without forgePersonId only when Hub is disabled.
    const message = err instanceof Error ? err.message : String(err);
    if (/not configured|Hub base URL/i.test(message)) {
      forgePersonId = "";
    } else {
      throw err;
    }
  ...
  const docRef = await addDoc(studentsRef, {
    ...payload,
    ...(forgePersonId ? { forgePersonId, forgeAcademyStudentId: "" } : {}),
    academyId,
  ...
  if (forgePersonId) {
    await updateDoc(docRef, { forgeAcademyStudentId: docRef.id });
  }
...
export async function updateStudent(studentId, input) {
  ...
  // forgePersonId / personId are Hub-owned and never writable from Academy forms.
...
function sanitizeStudentPayload(input) {
  return {
    ...
    // rmsPersonId may be set by Hub sync; forms may display but Hub owns forgePersonId.
    rmsPersonId: input.rmsPersonId?.trim() ?? "",
  };
  // Never accept client-supplied person identity keys in sanitize.
}
...
export class IdentityReviewRequiredError extends Error {
  ...
  constructor(review) {
    super(
      "A possible matching person already exists in Forge Identity. An administrator must resolve the duplicate review before creating this student.",
    );
