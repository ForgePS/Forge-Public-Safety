...
export default function StudentFormPage() {
  ...
  const [forgePersonId, setForgePersonId] = useState("");
      ...
      try {
        ...
        if (active) {
          setForm({
            ...
            rmsPersonId: student.rmsPersonId,
          });
          setProfilePictureUrl(student.profilePictureUrl ?? "");
          setForgePersonId(student.forgePersonId ?? "");
        }
      } catch (err) {
        if (active) {
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      } else if (err instanceof IdentityReviewRequiredError) {
        setError(
          `${err.message} Review id: ${err.review?.id || "pending"}. Resolve it under RMS Integration → Person Identity.`,
        );
      } else {
    ...
    <>
      <PageHeader
        ...
        actions={
          <div className="flex flex-wrap gap-2">
            {!isNew ? (
              <Link
                to={`/admin/settings/users/new?role=${ROLES.STUDENT}&studentId=${studentId}&email=${encodeURIComponent(form.email)}&displayName=${encodeURIComponent(`${form.firstName} ${form.lastName}`.trim())}`}
      ...
      <form className="flex flex-1 flex-col gap-5 p-6 lg:p-7" onSubmit={handleSubmit}>
        ...
        <FormSection title="Identity">
          ...
          {!isNew ? (
            <FormField
              label="Forge person ID"
              name="forgePersonId"
              value={forgePersonId || "—"}
              onChange={() => {}}
              disabled
              hint="Assigned by Forge Person Identity. Never enter or edit manually."
            />
          ) : (
            <p className="text-xs text-[var(--color-afta-muted)] md:col-span-2">
              Forge person ID is assigned automatically via the Integration Hub when this student is
              created. It cannot be entered or edited.
            </p>
          )}
        </FormSection>
        ...
        <FormSection title="Department">
          ...
          <FormField
            label="RMS person ID"
            name="rmsPersonId"
            value={form.rmsPersonId}
            onChange={handleChange}
            hint="Forge RMS platform personnel id for training sync."
          />
          <FormSelect
