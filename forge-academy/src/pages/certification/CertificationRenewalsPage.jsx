...
import {
  ...
  renewStudentCertification,
  STUDENT_CERTIFICATION_STATUS_LABELS,
} from "../../lib/studentCertifications.js";

export default function CertificationRenewalsPage() {
  const { user } = useAuth();
  ...
  async function reload() {
    setRows(await listRenewalDueCertifications());
  }
  ...
  async function handleRenew(certificationId) {
    if (!user?.uid) return;
    setActingId(certificationId);
    try {
      await renewStudentCertification({
        certificationId,
        officerUid: user.uid,
      ...
      setError(err instanceof Error ? err.message : "Unable to renew certification.");
    } finally {
      setActingId("");
    }
  }
  ...
  return (
    <>
      <PageHeader title="Renewals" subtitle="Expiring and expired certifications" />
            ...
            <table className="min-w-full text-left text-sm">
              ...
              <tbody>
                {loading ? (
                  ...
                  <tr><td colSpan={5} className="px-4 py-8 text-center text-[var(--color-afta-subtle)]">No renewals due.</td></tr>
                ) : (
                  rows.map((row) => (
                    <tr key={row.id} className="border-b border-[var(--color-afta-border)] text-[var(--color-afta-text)]">
                      ...
                      <td className="px-4 py-3">
                        <button type="button" disabled={actingId === row.id} onClick={() => handleRenew(row.id)} className="text-xs text-[#c8102e]">Renew</button>
                      </td>
                    </tr>
                  ))
                )}
